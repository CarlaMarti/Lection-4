...
class TestCleanDataFrame(unittest.TestCase):
    ...
    def test_dropna(self):
        ...
        self.assertFalse(cleaned_df.isnull().values.any(), "La función no elimina correctamente los valores nulos")

    def test_drop_duplicates(self):
        # Se eliminan los duplicados?
        cleaned_df = clean_dataframe(self.df)
        self.assertFalse(cleaned_df.duplicated().any(), "La función no elimina correctamente los duplicados")

    def test_row_count(self):
        # El numero de filas se actualiza correctamente? 
        cleaned_df = clean_dataframe(self.df)
        expected_rows = len(self.df.dropna().drop_duplicates())
        self.assertEqual(len(cleaned_df), expected_rows, "El número de filas no se actualiza correctamente")
